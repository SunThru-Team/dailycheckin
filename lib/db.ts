...
// ---- Types -----------------------------------------------------------------
export type Employee = { id: number; name: string; phone_number: string; email: string | null; access_token: string; active: boolean };
export type Call = {
...
export type CeoCall = { id: number; call_date: string; summary_text: string; twilio_call_sid: string | null; status: string | null };
...
// ---- CEO calls -------------------------------------------------------------
export const upsertCeoCall = async (dateISO: string, summary: string) =>
  (await sql<CeoCall[]>`
    INSERT INTO ceo_calls (call_date, summary_text, status) VALUES (${dateISO}, ${summary}, 'queued')
    ON CONFLICT (call_date) DO UPDATE SET summary_text = EXCLUDED.summary_text, status = 'queued'
    RETURNING id, call_date::text AS call_date, summary_text, twilio_call_sid, status`)[0];
...
export const getCeoCall = async (id: number) =>
  (await sql<CeoCall[]>`SELECT id, call_date::text AS call_date, summary_text, twilio_call_sid, status FROM ceo_calls WHERE id = ${id}`)[0] ?? null;

export const listCeoCalls = (limit = 30) =>
  sql<CeoCall[]>`SELECT id, call_date::text AS call_date, summary_text, twilio_call_sid, status
    FROM ceo_calls ORDER BY call_date DESC LIMIT ${limit}`;

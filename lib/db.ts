...
export type Employee = {
  ...
  role: 'ceo' | 'leadership' | 'employee';
};
export type CeoSchedule = { id: number; call_time: string; days: number[]; active: boolean; briefing_call_enabled: boolean };
export type Call = {
...
/** Everyone who receives check-in calls. The CEO is never called for a check-in. */
export const listCallableEmployees = () =>
  sql<Employee[]>`SELECT * FROM employees WHERE active AND role <> 'ceo' ORDER BY name`;

/** Ordered for the company view: CEO, then leadership, then everyone else. */
export const listPeople = () =>
  sql<Employee[]>`SELECT * FROM employees WHERE active
    ORDER BY CASE role WHEN 'ceo' THEN 0 WHEN 'leadership' THEN 1 ELSE 2 END, name`;

export const getEmployee = async (id: number) =>
  (await sql<Employee[]>`SELECT * FROM employees WHERE id = ${id}`)[0] ?? null;

export const getCeo = async () =>
  (await sql<Employee[]>`SELECT * FROM employees WHERE role = 'ceo' AND active ORDER BY id LIMIT 1`)[0] ?? null;

/** Resolve "Jeremy" to a person: exact, then prefix, then substring. */
export async function findPerson(needle: string) {
  const q = needle.trim().toLowerCase();
  if (!q) return null;
  const rows = await sql<Employee[]>`SELECT * FROM employees WHERE active ORDER BY name`;
  return rows.find((r) => r.name.toLowerCase() === q)
    ?? rows.find((r) => r.name.toLowerCase().split(/\s+/)[0] === q)
    ?? rows.find((r) => r.name.toLowerCase().startsWith(q))
...
export const listCeoSchedules = () =>
  sql<CeoSchedule[]>`SELECT id, call_time::text AS call_time, days, active, briefing_call_enabled
    FROM ceo_schedules WHERE active ORDER BY call_time`;

export const setBriefingCallEnabled = (enabled: boolean) =>
  sql`UPDATE ceo_schedules SET briefing_call_enabled = ${enabled}`;
